...
export default function AddUnitModal({ isOpen, onClose, onSuccess, units = [] }: AddUnitModalProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const response = await organizationalChartApi.createUnit({
        code: formData.code,
        name: formData.name,
        type: formData.type,
      ...
      if (response.success) {
        showSuccess('Unit berhasil dibuat');
        setFormData({
          code: '',
          name: '',
          type: 'unit',
      ...
      if (errorData?.errors) {
        ...
        setError(getErrorMessage(err) || 'Gagal membuat unit');
      }
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
        <div className="px-2 pr-14">
          ...
          <p className="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
            Buat unit baru untuk organizational chart. Unit dapat memiliki parent unit untuk membentuk hierarki.
          </p>
        </div>
